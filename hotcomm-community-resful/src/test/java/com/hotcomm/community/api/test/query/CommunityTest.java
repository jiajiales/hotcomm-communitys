...
public class CommunityTest {
    ...
    public static void loginToToken() throws Exception {
        ...
        token = user.getToken();
        List<CommunityDetailListUM> communityDetailList = user.getCommunityDetailList();
        if (communityDetailList == null || communityDetailList.size() == 0) {
            return;
        }
        Optional<CommunityDetailListUM> optional = communityDetailList.stream().filter((e) -> e.getIsDefault() == 2).findFirst();
    ...
    public void checkHttpResponse(HotHttpResponse response) {
        if(!response.isSuccess()){
            throw new RuntimeException("执行请求失败");
        }
        String returnJson = response.getReturnJson();
        Map<String, Object> ks = (Map<String, Object>) JSON.parse(returnJson);
        if (!ks.get("code").equals("0")) {
            throw new RuntimeException("执行请求失败");
        }
    }
